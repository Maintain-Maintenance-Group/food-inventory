// Lockers: same UI/logic as Pantry, but DB path = 'lockers'
...
function setupListeners() {
  const rootRef = ref(db, 'lockers'); // <— DB path for this page
  let state = {};

  onValue(rootRef, snap => { state = snap.val() || {}; render(state); });
  ...
  function render(data) {
    ...
    Object.entries(data).forEach(([folder, items]) => {
      ...
      items.forEach((item, i) => {
        ...
        nameInput.onchange = () => { items[i].name = nameInput.value; set(rootRef, data); };
        ...
        qty.onchange = () => { items[i].quantity = parseInt(qty.value, 10) || 0; set(rootRef, data); };
        row.appendChild(qty);

...
// - Renders folders/items
// - EXP field (MM-YY or MM-DD-YY) with expiry highlighting
// - Fill-level buttons (Empty / Half / Full)
// - Firebase auth + DB I/O
// Usage: import { mountInventoryPage } from './inventory.js'
...
// Keep which folders are currently open (per page instance)
...
export function mountInventoryPage({ dbRoot, containerId }) {
  ensureFirebase();
  ...
  // Auth UI elements (shared IDs in all pages)
  ...
  onAuthStateChanged(auth, user => {
    ...
    if (!addFolderBtn || !container) {
      console.error(`[inventory] Missing elements: addFolder or container #${containerId}`);
      return;
    }
    if (user) {
  ...
  function setupListeners() {
    const rootRef = ref(db, dbRoot); // 'pantry' | 'fridge' | 'lockers'
    let state = {}; // { folderName: [ {name, quantity, expires?, level?} ] }
    ...
    function render(data) {
      ...
      Object.entries(data).forEach(([folder, items]) => {
        ...
        items.forEach((item, i) => {
          ...
          exp.placeholder = 'MM-DD-YY';
          exp.maxLength = 12;
          exp.className = 'exp-input';
          exp.style.width = '12ch';
          ...
          exp.addEventListener('change', () => {
            ...
            items[i].expires = iso; // '' | 'YYYY-MM' | 'YYYY-MM-DD'
            set(rootRef, data);
